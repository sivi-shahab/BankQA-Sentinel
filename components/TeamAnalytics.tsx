    ...
    <div className="space-y-8 animate-fade-in pb-20 relative">
      ...
      <div className="bg-white rounded-[32px] p-8 border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tight">Team Intelligence</h1>
            <p className="text-slate-500 font-medium text-sm">Agent Performance & Compliance Oversight</p>
          </div>
        </div>
        
        {/* Quick Stats */}
        <div className="flex gap-4">
             <div className="px-6 py-3 bg-slate-50 border border-slate-200 rounded-2xl flex flex-col items-center">
                 <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Calls</span>
                 <span className="text-xl font-black text-slate-800">{TEAM_STATS.totalCalls}</span>
             </div>
             <div className="px-6 py-3 bg-slate-50 border border-slate-200 rounded-2xl flex flex-col items-center">
                 <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Avg Quality</span>
                 <span className="text-xl font-black text-emerald-600">{TEAM_STATS.avgQualityScore}%</span>
             </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-[32px] p-8 border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-lg font-black text-slate-800">Performance Leaderboard</h3>
                </div>
                <div className="flex items-center gap-2">
                    <Search className="w-4 h-4 text-slate-400" />
                    <input type="text" placeholder="Search agents..." className="bg-transparent text-sm font-bold outline-none w-32 focus:w-48 transition-all" />
                </div>
            </div>
            ...
            <div className="space-y-4">
                {MOCK_AGENTS.map((agent, index) => (
                    <div 
                        ...
                        className="p-4 bg-white border border-slate-100 rounded-2xl shadow-sm hover:border-indigo-200 hover:shadow-md transition-all cursor-pointer flex items-center justify-between group"
                    >
                        <div className="flex items-center gap-4">
                             <div className="text-lg font-black text-slate-300 w-6">#{index + 1}</div>
                             <AgentAvatar name={agent.name} skill={agent.topSkill} fallback={agent.avatar} />
                             <div>
                                 <h4 className="text-sm font-black text-slate-800 group-hover:text-indigo-600 transition-colors">{agent.name}</h4>
                                 <div className="flex items-center gap-2">
                                     <span className={`text-[9px] px-2 py-0.5 rounded-full font-black uppercase border ${
                                         agent.status === 'Active' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 
                                         agent.status === 'In Training' ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-red-50 text-red-600 border-red-100'
                                     }`}>{agent.status}</span>
                                     <span className="text-[9px] font-bold text-slate-400 flex items-center gap-1">
                                        <Star className="w-3 h-3 text-amber-400 fill-current" /> {agent.topSkill}
                                     </span>
                                 </div>
                             </div>
                        </div>
                        ...
                        <div className="flex items-center gap-6">
                            <div className="text-center">
                                ...
                                <div className={`text-lg font-black ${agent.avgScore >= 90 ? 'text-emerald-500' : 'text-slate-700'}`}>{agent.avgScore}</div>
                            </div>
                            <div className="text-center hidden sm:block">
                                <div className="text-[9px] font-black text-slate-300 uppercase">Calls</div>
                                <div className="text-lg font-black text-slate-700">{agent.callsCount}</div>
                            </div>
                             <div className="text-center hidden sm:block">
                                <div className="text-[9px] font-black text-slate-300 uppercase">Compliance</div>
                                <div className={`text-lg font-black ${agent.complianceRate >= 95 ? 'text-emerald-500' : 'text-amber-500'}`}>{agent.complianceRate}%</div>
                            </div>
        ...
        <div className="space-y-8">
            ...
            <div className="bg-white rounded-[32px] p-8 border border-slate-200 shadow-sm">
                <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest mb-6 flex items-center gap-2">
                    <Target className="w-4 h-4 text-indigo-500" /> Competency Matrix
                </h3>
                <div className="h-[250px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                        <RadarChart cx="50%" cy="50%" outerRadius="80%" data={TEAM_STATS.competencyAverages}>
                            ...
                            <Radar name="Team Avg" dataKey="A" stroke="#6366f1" strokeWidth={3} fill="#6366f1" fillOpacity={0.2} />
                            <Tooltip />
                        </RadarChart>
                    </ResponsiveContainer>
                </div>
            ...
            <div className="bg-white rounded-[32px] p-8 border border-slate-200 shadow-sm">
                 <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest mb-6 flex items-center gap-2">
                    <ThumbsUp className="w-4 h-4 text-emerald-500" /> Sentiment Pulse
                </h3>
